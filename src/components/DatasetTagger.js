...
datasetRow(dataset){
  ...
  return [dataset.name, fieldTags["PII"] || "FALSE", fieldTags["SENSITIVE"] || "FALSE", dataset.fields]
}

fetchDatasets(){
  axios.get('/governance/api/v1/datasets').then((response) => {
    const datasetsData = response.data
    const datasets = datasetsData.datasets.map(dataset => this.datasetRow(dataset))
