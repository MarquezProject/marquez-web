...
class DatasetTaggerDetailsDialog extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        if(this.props.dataset !== prevProps.dataset) {
          ...
          this.fieldsForDataset(this.props.dataset).map(field => this.initTagsForField(field.name, field.tags))
        }
    }
    ...
    handleAddButtonPress(event, fieldName){
        ...
        axios.post("/governance/api/v1/datasets/"+datasetName+"/fields/"+fieldName+"/tags/"+tagName)
            .then(res => {
                console.log(res);
    ...
    handleDeleteButtonPress(fieldName, tagName){
        ...
        const datasetName = this.props.dataset[0]
        axios.delete("/governance/api/v1/datasets/"+datasetName+"/fields/"+fieldName+"/tags/"+tagName)
            .then(res => {
                console.log(res);
                console.log(res.data);
        })
    ...
    fieldTag(fieldName, tag){
        return (
            <TableRow key={"field-"+fieldName+"-tag-"+tag}>
                ...
                <TableCell>
                    <RemoveCircleIcon onClick={() => this.handleDeleteButtonPress(fieldName, tag)} />
                </TableCell> 
            </TableRow>
        )
    }
        ...
        return (
            <TableRow key={"field-row-" + datasetField.name}>
                ...
                <FormControl className={styles.formControl}>
                    <InputLabel
                        htmlFor="outlined-allowed-tags"
                    >
                    </InputLabel>
                    <Select
                        value={this.state[this.stateKeyForField(datasetField.name)] || "None"}
                    ...
                    >
                        ...
                        <MenuItem key="pii" value="PII">PII</MenuItem>
                        <MenuItem key="sensitive" value="SENSITIVE">SENSITIVE</MenuItem>
                    </Select>
                    </FormControl>
                    {
                        this.isTagSelectedForField(datasetField.name) && 
                        <Button variant="contained" color="primary" onClick={(evt) => this.handleAddButtonPress(evt, datasetField.name)}>
                            ADD
                        </Button>
                    }
                </TableCell>
    ...
    render(){
        return (
            ...
            >
                ...
                <DialogContent>
                    <h2>
                        {this.state.dataset !== undefined && this.state.dataset[0]}
                    </h2>
                    <Table>
                        <TableBody>
                            <TableRow>
                                <TableCell><b>FIELD NAME</b></TableCell>
                                <TableCell><b>FIELD TYPE</b></TableCell>
                                <TableCell><b>FIELD TAGS</b></TableCell>
                                <TableCell><b>ADD TAG</b></TableCell>
                            </TableRow>
                            {this.state.dataset !== undefined && 
                             this.fieldsForDataset(this.state.dataset).sort(
                                function(a, b){
